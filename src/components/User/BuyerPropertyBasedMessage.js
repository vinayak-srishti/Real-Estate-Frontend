...
const params = useParams();
  ...
  useEffect( ()  => {
    async function fetch() {
      var buyer = await axiosInstance.get(
        "/user/profileView/" + localStorage.getItem("userId"),
        {
          headers: {
    ...
    setInterval(async () => {
      ...
      for (var i in res.data) {
        if (
          res.data[i].buyerId == localStorage.getItem("userId") &&
          res.data[i].sellerId == sendMsg.sellerId &&
          res.data[i].propertyId == sendMsg.propertyId
        ) {
  ...
  return (
    <section className="container msger mt-5 pt-5">
      <main className="container msger-chat">
        {msg.map((msgContainer, index) =>
          msgContainer.messagefrom && msgContainer.messagefrom == "buyer" ? (
            <div className="msg right-msg">
              <img className="msg-img" src={imgRight} />
            ...
            <div className="msg left-msg">
              <img className="msg-img" src={imgLeft} />
              <div className="msg-bubble">
                <div className="msg-info">
